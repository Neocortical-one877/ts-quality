...
- Hardened attestation signing and verification behind one shared render-safe contract: signer-side run metadata must now agree with the signed subject path, zero-width/BOM/invisible Unicode format characters are rejected alongside control and bidi spoofing characters, symlinked subjects that resolve outside `--root` are rejected during signing and verification, CLI option parsing now fails closed on unknown options instead of swallowing them as metadata values, and forged-attestation tests now reuse production canonical signing bytes.
