...
- Downstream decision commands (`plan`, `govern`, `authorize`) project from the latest evaluated run plus exact run-bound approvals/attestations. `check` snapshots the decision control plane (config digest, policy defaults, constitution rules, agent grants, and support-path bindings), so later decision surfaces keep using that run-bound snapshot and refuse the request if the analyzed changed files or snapped config / constitution / agents drift after `check`.
